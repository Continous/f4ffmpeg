...
namespace f4ffmpeg
{
        ...
        const char* texturePath)
    {
        ...
        // Remove "Textures\\" but keep f4se_rttv and any subdirectories beneath it.
        normalized.erase(
            0,
            std::string_view{"textures\\"}.size()
        );

        // Remove the marker suffix. The filename stem itself becomes the video filename.
        normalized.erase(
            normalized.size() -
    ...
    bool initializeNifHandler()
    {
        ...
        std::call_once(
            ...
            []()
            {
                // BSTextureSet declares the two texture-acquisition vfuncs at slots 0x2A and 0x2B. Hook both on the concrete BSShaderTextureSet vtable so prefetched and ordinary NIF texture acquisition are covered.
                ...
                {
                    REX::ERROR(
                        "Failed to resolve f4ffmpeg BSShaderTextureSet interception targets."
                    );
